...
class plugin_userstats extends plugin_base {
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        if ($data->stat === 'coursededicationtime') {
            $courseid = $this->resolve_time_tracking_courseid((int)$courseid, $row);
            if ($courseid <= 0) {
                return 0;
            }
            $sql = "userid = ?";
            ...
            if ($logs) {
                ...
                if (!empty($totalsbyday)) {
                    ...
                    $realdedication = 0;
                    foreach ($totalsbyday as $dailytotal) {
                        $realdedication += max(0, (int)$dailytotal);
                    }

                    if ($realdedication > 0) {
                        return format_time($realdedication);
                    }
                }
    ...
    /**
     * Resolve a concrete course ID for time tracking metrics.
     ...
     * @return int
     */
    protected function resolve_time_tracking_courseid(int $courseid, object $row): int {
